...
| 2026-06-04 | tasks.created_at como TIMESTAMPTZ en vez de BIGINT | `EXTRACT(EPOCH FROM now())::BIGINT * 1000` rompe el parser del SQL Editor de Supabase. TIMESTAMPTZ es consistente con todas las demás tablas. Task.created_at pasa de `number` a `string` en TypeScript. | BIGINT con ms epoch (probado: rompe parser; y añade inconsistencia de tipos con el resto de tablas) |
